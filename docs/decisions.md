...
- **Claude Code writes both sides of the thread,** from the maintainer's account through the GitHub connector: a line comment with a suggested change on a merged pull request (#47), and the author's reply. Each ends with "Generated by Claude Code", and the captions say who wrote it. This replaces item 4 of "The screenshot plan's open decisions, settled" (the maintainer leaving the comment).
- **The comment is a suggested edit** (GitHub's suggestion box), on a real gap: the quick reference in `version-control` left out `git stash list`. The fix went into the pull request that added the figures, as the reply says.
- **Names and avatars are blurred** in both figures, with `tools/shots`'s new `blur:` key, although the account is the maintainer's. A figure that must show people's names or faces blurs them and says so in its caption.

**Why.** The maintainer answered in conversation on 2026-09-25: the request had been lost among other updates, and writing the thread in the session unblocked two figures without waiting on anyone. A real thread on the book's own repository stays honest; the "Generated by Claude Code" line and the caption keep it labeled.

**Where.** `tools/shots/recipes/version-control.yml` and `collaboration.yml`, `tools/shots/lib/blur.py`, `AGENTS.md` ("Screenshots", "Honest and private"), the [review thread](https://github.com/cuinfoscience/INFO-Missing-Manual/pull/47#discussion_r4100343519).
...
- **The review thread (item 4):** *superseded 2026-09-25* (see "The review-thread screenshots" above). The maintainer leaves a real line comment on a merged pull request in this repository. The `version-control` diff figure shows it, and the same thread, once replied to and resolved, can serve `collaboration`.
