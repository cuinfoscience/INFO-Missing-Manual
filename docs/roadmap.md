...
- [ ] **Worked examples that are still outlines.** Four chapters had one-line bullet outlines under "Worked examples (outline)": `version-control`, `collaboration`, and `project-management` (written out in September 2026; `version-control`'s and `project-management`'s with real transcripts), then `automation`. One pull request per chapter.
