...
- [ ] **Screenshots for the twelve placeholders**, then the interface-heavy chapters, with `tools/shots`: [`plans/2026-09-24-screenshots.md`](plans/2026-09-24-screenshots.md). The pilot (M1) replaced three in the pull request that fixed #30: two JupyterLab views in `jupyter` and the rendered DataFrame in `pandas-basics`. Nine remain, in seven chapters (M2–M5).
