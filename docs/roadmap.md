...
- [ ] **Worked examples that are still outlines.** Four chapters had one-line bullet outlines under "Worked examples (outline)": `version-control` and `collaboration` (written out in September 2026; `version-control`'s with real transcripts from a practice repository), then `project-management` and `automation`. One pull request per chapter.
