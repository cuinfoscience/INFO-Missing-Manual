...
- [ ] **Wide tables on phones.** At 390 px, three pages scroll sideways because of tables wider than the screen (`file-system`, `jupyter`, `writing-thesis`); Part I's rewrite fixed `common-formats` and `artifacts-have-politics` in #62. `tools/layout-audit/audit.py widths --widths 390` lists them. A fix must not change the desktop layout.
