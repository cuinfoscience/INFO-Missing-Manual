...
- [ ] **Worked examples that are still outlines.** Four chapters had one-line bullet outlines under "Worked examples (outline)": `version-control` (written out in September 2026, with real transcripts from a practice repository), `collaboration`, `project-management`, and `automation`. One pull request per chapter.
