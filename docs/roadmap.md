...
- [ ] **Wide tables on phones.** At 390 px, five pages scroll sideways, four because of tables wider than the screen (`common-formats`, `file-system`, `jupyter`, `writing-thesis`) and one in `artifacts-have-politics`. `tools/layout-audit/audit.py widths --widths 390` lists them. A fix must not change the desktop layout.
