...
- [x] **A welcoming narrative voice in every chapter** ([`decisions.md`](decisions.md), 2026-09-25; rules in `AGENTS.md`, "Tone and Voice"). One pull request per part, after the pilot, following [`plans/2026-09-25-voice-rollout.md`](plans/2026-09-25-voice-rollout.md); each also replaces the chapter's "Learning objectives" with "Why read this chapter" bullets:
...
- [ ] **Revise the book after the September 2026 peer review** ([`plans/2026-09-25-peer-review-revisions.md`](plans/2026-09-25-peer-review-revisions.md), from [`aar/2026-09-25-peer-review.md`](aar/2026-09-25-peer-review.md)). Waiting on the maintainer's eight decisions; then:
    ...
    - [ ] Phase 5: new chapters, as approved (charts and figures; sharing, archiving, and data ethics; writing at work)
...
- [ ] **Data ethics and licensing** — possible new chapter or section in `artifacts-have-politics.qmd` covering data licenses (CC-BY, ODbL, terms of use), citation of datasets, and the ethics of scraping vs. downloading from a published source. The peer-review plan proposes a new chapter on sharing, archiving, licensing, and data ethics (its decision 3, Phase 5); decide the two together.
...
- [x] **The book's own CI uses older action versions.** `.github/workflows/build-book.yml` pinned `actions/checkout@v4` and `actions/setup-python@v5`, while `automation` (since #67) teaches `@v7`, the current majors in September 2026. Updated to `@v7` in the Phase 1 pull request, after `git ls-remote` confirmed the tags and actionlint passed; `quarto-dev/quarto-actions` is still at `v2`. The publish step runs only on `main`, so the first deploy after the merge is its real test.
- [ ] **Re-run every chapter's examples once a year.** Tool changes (pandas 3, git 2.43, ruff 0.16, the Anthropic SDK dropping `temperature=`) caused many of the voice rewrite's fixes ([AAR](aar/AAR_INFO-Missing-Manual_2026-09-25.md)). A yearly pass with the rollout's checks, alongside the fall re-check of the "At CU Boulder" callouts and `jupyter`'s hosted-notebook limits, would catch the next round.
...
Since then, work has gone into existing chapters rather than new ones: deeper Markdown coverage in `common-formats.qmd` (#27) and an "Opening a terminal" walkthrough in `terminal.qmd` (#28); in September 2026, real screenshots (#37–#41, #50), the sections in "Chapter and section candidates" above (#46–#49, #52–#56), filled-in worked examples (#57–#60), and a rewrite of every chapter in the book's current voice (#61–#68). A simulated peer review followed the rewrite, and its [revision plan](plans/2026-09-25-peer-review-revisions.md) is where new chapters may come from next: a Day zero setup appendix, charts and figures, sharing and archiving data, and writing at work, each waiting on the maintainer.
