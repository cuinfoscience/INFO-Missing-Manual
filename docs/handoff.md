...
**Updated 2026-09-25,** with Part VI's voice rewrite open as #67. This note describes the present state. Rewrite it when a session stops or the state changes, and don't let it grow into a history: history lives in git, in [`decisions.md`](decisions.md), and in the AARs.
...
2. **The voice rewrite, next for agents** ([`roadmap.md`](roadmap.md), planned work): Part VII (Algorithmic Systems), with the appendices, introduction, and conclusion, next, after #67 merges; that finishes the voice rewrite. Follow [`plans/2026-09-25-voice-rollout.md`](plans/2026-09-25-voice-rollout.md): one agent per chapter from its brief, then review each chapter, render, and run the layout audits (including `widths --widths 390`) before its commit. Every roadmap topic is done except data ethics and licensing, which waits on the maintainer.
