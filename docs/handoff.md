...
**Updated 2026-09-24,** after #51 merged. This note describes the present state. Rewrite it when a session stops or the state changes, and don't let it grow into a history: history lives in git, in [`decisions.md`](decisions.md), and in the AARs.
...
2. **New sections, next for agents,** in the roadmap's order, one pull request each: data ethics and licensing (a section in `artifacts-have-politics`, or a new chapter: the roadmap leaves it open, so ask the maintainer), then the rest of the roadmap.
