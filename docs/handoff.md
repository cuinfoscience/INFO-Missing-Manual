...
2. **The voice rewrite is done** (#61–#68); [`aar/AAR_INFO-Missing-Manual_2026-09-25.md`](aar/AAR_INFO-Missing-Manual_2026-09-25.md) reviews it. Then the peer review ([`aar/2026-09-25-peer-review.md`](aar/2026-09-25-peer-review.md)) and its revision plan: Phase 1 can start now; the rest waits on the maintainer's decisions. Also next for agents: update the book's own CI to the GitHub Actions versions `automation` teaches (roadmap), then a yearly re-run of every chapter's examples, since tool changes caused many of the rewrite's fixes.
