...
**Updated 2026-09-24,** after #46 merged. This note describes the present state. Rewrite it when a session stops or the state changes, and don't let it grow into a history: history lives in git, in [`decisions.md`](decisions.md), and in the AARs.
...
2. **New sections, next for agents,** in the roadmap's order, one pull request each: cloud notebooks (Colab, Kaggle, Codespaces), then out-of-memory data (chunked reads, Polars, DuckDB).
