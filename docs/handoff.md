...
- **Open pull request from `claude/markdown-chapter-expansion-difpcc`:** the `automation` screenshot (this book's CI job, signed out), the reordered roadmap, and these records.
...
| The JSON figure for `http-apis` | A hand capture of `https://api.github.com/repos/pandas-dev/pandas` in Chrome with "Pretty-print" ticked: at most about 1024×768, the top of the response, no bookmarks bar or account details. An agent records it with `tools/shots/run adopt` and writes the caption. |
...
- **Seven `PLACEHOLDER-*` images** don't exist, in six chapters (`operating-system` 2, and one each in `version-control`, `collaboration`, `http-apis`, `linting`, `text-editors`), so those figures render broken.
...
1. **Screenshots, while they wait on the maintainer:** the `version-control` diff (after the review comment), `http-apis` (after the hand capture), then `collaboration` from the same thread once it has a reply and is resolved.
