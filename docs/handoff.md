...
**Updated 2026-09-24,** after #57 merged. This note describes the present state. Rewrite it when a session stops or the state changes, and don't let it grow into a history: history lives in git, in [`decisions.md`](decisions.md), and in the AARs.
...
2. **New sections:** every roadmap candidate is done except data ethics and licensing, which waits on the maintainer's choice of placement (above). Under way: writing out the Worked examples that are still one-line outlines (roadmap, open follow-ups), one chapter per pull request: `version-control` (done), `collaboration` (open), then `project-management` and `automation`.
