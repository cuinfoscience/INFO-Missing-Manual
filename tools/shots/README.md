...
- **GitHub review threads:** open the comment's permalink (`…/pull/<n>#discussion_r<id>`) on the Conversation tab. It shows the diff lines the comment is anchored to, the comment, and a suggestion's *Suggested change* box in one element, `review-thread-collapsible`, which crops cleanly with `pad: 8`. At 800 px GitHub's sidebar squeezes the thread to 376 px; at 1024 it is about 520 px, fine in the body column. Scroll it 72 px below the top, clear of the sticky "Merged" bar. A resolved thread opened by its permalink comes up expanded, with *Hide resolved* in its header; from the plain pull request URL it stays collapsed to one line.
- **A subject that changes state can't be retaken as it was.** The diff figure was first taken before the thread's reply, and the recipe could no longer reproduce it once the thread was resolved. Write the recipe so a retake still shows what the figure needs in the new state (here, `crop: {between: [thread, first comment]}`), retake, and promote that take.
- **Blur people's names and faces** with `blur:`, even on an account that is yours to show: it keeps the rule simple. List every selector that can show a name in the crop (on GitHub, `img.avatar` and `a.author` inside the thread), look at the take, and say "blurred" in the caption.
...
- **Blurring names and faces:** `blur: ['.author', 'img.avatar']` blurs every element the selectors match (a 6-pixel CSS blur, `lib/blur.py`) after the steps run and before the screenshot, and the take and its provenance record the selectors. A selector that matches nothing fails the take, so a redesign can't quietly leave a name in frame. Headless takes only. Say in the caption that the figure is blurred.
...
- **Hand captures and older images:** `adopt` records an image made outside the toolkit, from its recipe's `legacy:` block (`captured:` date, `method:`, and optionally `by:`, who made it; without `by:` the record says "hand-run script, before tools/shots"). Say in `method:` how the image was cropped, if it was; `recipes/http-apis.yml` is an example. Use it for the operating-system screenshots that need a real macOS or Windows machine.
