...
**Remaining `PLACEHOLDER-*` images.** Eight placeholder PNGs, in seven chapters, still don't exist: VS Code (×2, `text-editors` and `linting`), GitHub's web interface (×3: the pull-request diff in `version-control`, `collaboration`, `automation`), the Windows and macOS About panels (×2, `operating-system`), and a browser's JSON view (`http-apis`). The other four are real captures since September 2026: two JupyterLab views, a rendered DataFrame, and this book's repository page. Unlike terminal sessions, these cannot be honestly simulated; they are real captures, made with `tools/shots` (next section) or, for the operating-system panels, by hand on a real machine. Which ones, in what order, is in `docs/plans/2026-09-24-screenshots.md`.
