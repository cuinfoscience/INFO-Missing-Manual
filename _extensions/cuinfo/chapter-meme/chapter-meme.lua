...
return {
  ["chapter-meme"] = function(args, kwargs, meta)
    ...
    -- memegen sizes captions to each template's authored text boxes, so
    -- there is no fontsize knob. The width and font defaults match the
    -- generator script; folding both into the spec hash below means
    -- changing either (here or per-chapter via `meme.width:` /
    -- `meme.font:`) invalidates the cached PNG.
    local width = m.width and pandoc.utils.stringify(m.width) or "1000"
    local font = m.font and pandoc.utils.stringify(m.font) or "impact"
    local lines = {}
