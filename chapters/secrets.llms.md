...
**First, rotate the key.** At the service that issued it, revoke the old key and create a new one. It’s the only step that stops the damage: once the old key is dead, it doesn’t matter who copied it. Do it even if the repository is private, and even if you deleted the file a minute later, because the key is still in the history.
...
**Finally, think about what the key could reach.** A weather key costs a quota. A write token, a cloud key, or a database URL may mean checking access logs and billing, and telling your team, instructor, or IT staff.

## 34.10 Stakes and politics
