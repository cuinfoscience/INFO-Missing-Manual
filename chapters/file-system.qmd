...
meme:
  ...
  alt: "No Idea Dog Meme: Where Did I Save That File?"
# margin-header: generated from meme: by tools/chapter-meme/sync_margin_header.py; do not edit
margin-header: |
  ![](/graphics/memes/file-system.png){.chapter-meme fig-alt="No Idea Dog Meme: Where Did I Save That File?"}
---
...
**See also:** @sec-terminal, @sec-scripts-vs-notebooks, @sec-git-github, @sec-project-management.
:::
...
The macOS equivalent of OneDrive's "files on demand" behavior is **iCloud Drive optimization**. With *Optimize Mac Storage* on, macOS moves documents you haven't opened lately to iCloud when it needs space, leaving placeholders that download again when you open them. Same problem, same fix, both covered in @sec-filesystem-cloud: for any folder where you actually run code, make sure the files are really on your Mac.
...
| **Backup** | Keeps earlier versions, in a copy separate from your working files | Deletions, overwritten files, a bad edit last week, ransomware | Nothing, if you have never tested a restore (@sec-os-management) |
...
To save disk space, sync clients can leave a [**placeholder**](appendix-glossary.qmd#term-online-only-file) on your disk: the file's name and icon appear in File Explorer or Finder, but its contents stay in the cloud until something opens it. OneDrive does this by default on current Windows, and macOS does it for iCloud Drive when *Optimize Mac Storage* is on. For documents this is convenient. For code it is a trap: your script asks for `data/raw/survey.csv`, the operating system has to download it first, and the read either stalls or fails with a confusing error, especially when you are offline.
...
That has two consequences for technical work. First, everything you save in Documents is synced, including project folders you never meant to sync. Second, the path now contains spaces and punctuation, which breaks any command that doesn't quote it (@sec-terminal):
...
- **Virtual environments** (`.venv/`, conda environments). They hold tens of thousands of files and are tied to the computer that made them, so they sync slowly and break on the other machine. They can be rebuilt from `requirements.txt` anyway (@sec-virtual-environments).
- **Git repositories.** Git and the sync client each assume they are in charge of the folder. When they disagree (two computers, a half-finished sync), you can get conflicted copies inside the hidden `.git/` folder and a damaged repository. Let GitHub be the way a repository moves between computers (@sec-git-github).
...
`Path(__file__).resolve().parent` is the trick that makes a script find its own folder no matter where it is launched from: the script asks the operating system where its own source file lives, resolves any symlinks, and uses that as the anchor for all subsequent paths. Every relative path is then built by joining with the `/` operator, which `pathlib` translates into whatever separator the current operating system wants. (See @sec-scripts-vs-notebooks for when to bring this discipline from notebooks into reusable scripts, and @sec-git-github for how the project layout interacts with version control — `data/raw/` typically belongs in `.gitignore`, while `src/` and `notebooks/` do not.)
...
See @sec-artifacts-politics for the broader framework. The concrete prompt to carry forward: when you decide where a project lives, ask whose rules the storage runs under and what happens when the network is gone.
...
6.  Find out whether your Documents folder is synced (OneDrive's folder backup on Windows, iCloud's Desktop & Documents Folders on macOS). If it is, choose a folder outside sync for your projects, write down its full path, and check one data file with the size comparison in @sec-filesystem-cloud.
