...
For any of these, `ssh -v username@hostname` turns on verbose output so you can see which stage is failing. See @sec-asking-questions if you need to escalate to your IT department.
:::
...
The other common approach is the older "edit locally, sync, run remotely" loop: you edit your project on your laptop, push changes to a Git remote (@sec-git-github), pull them on the server, and run them there. This works and is fully reproducible, but it has a minor latency penalty per edit-run cycle compared to the remote-editor pattern.
