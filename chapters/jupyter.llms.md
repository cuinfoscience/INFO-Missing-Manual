...
The one rule of cell order that you should treat as inviolable is: **keep imports and configuration near the top**. Every notebook should begin with a small block of imports (`import pandas as pd`, etc.), a path-and-parameter setup cell, and a smoke-test cell (see [sec-scripts-vs-notebooks](#sec-scripts-vs-notebooks)), and nothing else should come before them. If an import cell is buried five pages down, anyone running the notebook from the top will hit a `NameError` before they ever get there. All the imports, all at the top, once.
...
Third, **never embed secrets — API tokens, passwords, private URLs — directly in a notebook cell.** The notebook file is saved to disk with every cell’s contents intact, and when you commit it to git (or accidentally share it), whatever you typed is going with it. Load secrets from environment variables or from a `.env` file that is listed in `.gitignore` (see [sec-secrets](#sec-secrets)). This rule applies to both Python cells and shell cells; the file is the same.
...
The professional-level version is to maintain an environment file in the project that pins every dependency (see [sec-virtual-environments](#sec-virtual-environments) and [sec-pkg-mgmt](#sec-pkg-mgmt)). For a conda environment, that file is `environment.yml`; for a pip-based virtual environment, it is `requirements.txt` or a `pyproject.toml`. Whichever you use, commit it to version control alongside the notebook, and write one line in the `README.md` explaining how to recreate the environment from it. With an env file plus the code, anyone — including you on a new machine — can get back to the exact same stack and reproduce the results.
...
Second, **confirm the environment in a top-of-notebook cell** every single time. The smoke-test cell (see [sec-scripts-vs-notebooks](#sec-scripts-vs-notebooks)) should print `sys.executable` and the versions of any packages the notebook cares about:
...
The longer-term prevention is the one from [sec-filesystem](#sec-filesystem) and [sec-scripts-vs-notebooks](#sec-scripts-vs-notebooks): keep a stable project root and always launch Jupyter from it, so paths never depend on where the server was started.
...
|  | Google Colab | Kaggle Notebooks | GitHub Codespaces |
|----|----|----|----|
| **What it is** | A Jupyter-style notebook on a Google virtual machine | A notebook next to Kaggle’s datasets and competitions | A full development environment for one repository: VS Code in the browser, with notebooks, a terminal, and files |
| **Where the notebook is saved** | Your Google Drive | Your Kaggle account, as saved versions | The repository, once you commit and push |
| **What disappears** | Every file on the virtual machine, when the runtime is recycled | Everything but saved versions and their output | Nothing while the codespace exists; the codespace itself, after a period of disuse |
| **Packages** | Many preinstalled; your installs last one session | Many preinstalled; your installs last one session | Whatever the repository’s configuration installs, and anything you add |
| **Secrets** | The Secrets panel (the key icon) | *Add-ons → Secrets* | Codespaces secrets in your GitHub settings, as environment variables |
| **Good for** | Course notebooks, and trying a GPU | Working with Kaggle datasets and competitions | Projects in a repository, when your own computer can’t run them |
...
On all three, the habit from “Reproducibility discipline” above matters more, not less: expect to start from a fresh machine, so make *Restart and Run All* work from the first cell.

### The environment isn’t yours

Colab and Kaggle start every session from their own image, with hundreds of packages already installed, at versions the platform chose and changes without asking you. Two consequences. First, a package you install with `%pip install` lasts only until the session ends, so put the installs in the notebook’s first cell rather than typing them once. Second, a notebook that works today can break when the image updates. Pin the versions that matter (`%pip install pandas==2.2.3`) and record the rest by printing versions in the notebook, as in “Environment capture” above. A Codespace instead builds its environment from the repository’s own configuration, a `.devcontainer/devcontainer.json` file and usually a `requirements.txt`, which makes it the closest of the three to the virtual environments in [sec-virtual-environments](#sec-virtual-environments).

### Secrets go in the platform’s secrets store

Never paste an API key into a cell. Notebooks are made to be shared, and a key in a cell is shared with them, along with the outputs, which can print it too. Each platform has a secrets store that keeps the key outside the notebook:
...
3.  Add a first cell to one of your notebooks that prints the Python executable, the versions of the packages you import, and the date, as in “Environment capture” above.
