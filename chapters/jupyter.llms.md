...
The `python -m pip` in the middle is on purpose. It [installs into whichever Python `python` is](https://pip.pypa.io/en/stable/user_guide/#running-pip), so with the venv active, ipykernel can’t land anywhere else. Plain `pip` is a separate command that can belong to a different Python, and that mismatch is how this whole problem starts.
