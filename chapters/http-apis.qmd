...
Paste the same URL into a browser and you get the same response, often formatted for reading (@fig-browser-json). Either way, it is plain JSON: keys and values, with objects such as `owner` nested inside.
...
See @sec-artifacts-politics for the broader framework, including the CFAA and DMCA history. The concrete prompt to carry forward: when you build a project on someone else's API, ask what happens when the provider changes the rules — because they will.
