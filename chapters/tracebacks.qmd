...
meme:
  ...
  lines:
    - "NOT SURE IF MY BUG"
    - "OR THE LIBRARY'S BUG"
  alt: "Fry from Futurama Meme: Not Sure If My Bug Or The Library's Bug."
# margin-header: generated from meme: by tools/chapter-meme/sync_margin_header.py; do not edit
margin-header: |
  ![](/graphics/memes/tracebacks.png){.chapter-meme fig-alt="Fry from Futurama Meme: Not Sure If My Bug Or The Library's Bug."}
---
...
**Prerequisites (read first if unfamiliar):** @sec-debugging.

**See also:** @sec-scripts-vs-notebooks, @sec-asking-questions, @sec-latex.
:::
...
## Running theme: the last line tells you *what*, the stack tells you *where* {.unnumbered}
...
## Reading up, reading down: finding the frame that's yours
...
## The ten exceptions you'll see most often
...
The command is fine; it's in the wrong place. `pip install` is for your shell, the prompt that ends in `$`, `%`, or `>`, and `>>>` means you're talking to Python, which tries to read the line as Python code. Leave Python with `exit()` (or `Ctrl + D`; on Windows, `Ctrl + Z` and then Enter) and run the command at the shell prompt. In a Jupyter notebook, where every cell is Python, run `%pip install pandas` in a cell instead. @sec-terminal shows how to tell the prompts apart. (Python 3.13 and later name the file `<python-input-0>` instead of `<stdin>`.)
...
Jupyter prints tracebacks in its own style, with more source code and less punctuation, and the first time you see one it can look like a different language. The bones are the same. Here's the [notebook](appendix-glossary.qmd#term-notebook) version of a very common mistake, asking for a `date` column in a file whose column is really called `Date`:
...
See @sec-artifacts-politics for the broader framework. The concrete prompt to carry forward: before you paste a traceback anywhere, read it once as a stranger would, and redact whatever isn't part of the bug.
...
If you'd tried `df["reading"].astype(float)` instead, you'd have gotten `ValueError: could not convert string to float: 'no reading'`, which names the culprit directly. Different route, same cause. See @sec-data-file-formats for the other ways missing values hide in files.

### 3. A chained traceback from a web request

This script asks a web server for a user's profile and parses the reply as [JSON](appendix-glossary.qmd#term-json), wrapping any parsing failure in an error of its own:
...
-   When asking for help, paste the whole traceback plus the code, after removing anything private (see @sec-asking-questions).

## Quick reference: common exceptions → first suspect
