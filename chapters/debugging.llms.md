...
|----|----|----|
| Syntax (`SyntaxError`, `IndentationError`) | Python couldn’t read the file, so nothing ran: an unclosed bracket or quote, a missing colon, indentation that doesn’t line up | Look at the caret, then the line above |
...
A few techniques make shrinking faster. The most useful is to **replace your real data with a few values you make up.** Say a script that averages quiz scores from a 200-student gradebook reports a class average in the 50s, and you know it was in the 80s. Throw away the gradebook and keep three scores you can add in your head:
...
for i in range(1, len(scores)):
    total += scores[i]
print(total / len(scores))   # 57.0
```

Those three average 86.33, and the script prints `57.0`. With numbers this small you can do the arithmetic yourself: 92 + 79 is 171, and 171 / 3 is 57, so the loop never added the 88. `range(1, len(scores))` starts at position 1, and Python counts from 0, a classic [off-by-one error](https://en.wikipedia.org/wiki/Off-by-one_error). Looping over the items themselves (`for score in scores: total += score`) can’t skip one, and prints `86.33333333333333`.

The same trick works when the data comes in through pandas, which reads from an [`io.StringIO`](https://docs.python.org/3/library/io.html#io.StringIO) object exactly as it reads from a file, so a short string of inline CSV is enough to recreate most data-loading bugs:
...
A three-row DataFrame or a five-item list, hard-coded like these, is almost always enough to reproduce a logic bug, and small enough to reason about end to end. Two more habits help. **Delete code aggressively**; if removing a block doesn’t change the symptom, the block wasn’t involved. And **freeze any randomness.** If a bug only appears sometimes, fix the [random seeds](https://docs.python.org/3/library/random.html#random.seed) (`random.seed(0)`, `numpy.random.seed(0)`, `torch.manual_seed(0)`) so that “sometimes” becomes “every time on this seed,” which you can debug.
