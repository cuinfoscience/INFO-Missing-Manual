...
2. **Cleaning and data dictionary.** Turn the raw data into a clean, documented dataset. The milestone is "every column is typed correctly, missingness is explicitly handled, and a [data dictionary](appendix-glossary.qmd#term-data-dictionary) explains what each column means."
...
Because the dictionary is a file in your repository, its history is your schema's history: `git log -p data/dictionary.csv` shows every change to it, with the date and the commit message that explains why (see @sec-git-github).
