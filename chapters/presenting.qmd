...
meme:
  ...
  lines:
    ...
    - "PRACTICE TALK: 45 MINTUES"
  alt: "DBG Meme: Talk length: 15 minutes, Practice talk: 45 minutes."
# margin-header: generated from meme: by tools/chapter-meme/sync_margin_header.py; do not edit
margin-header: |
  ![](/graphics/memes/presenting.png){.chapter-meme fig-alt="DBG Meme: Talk length: 15 minutes, Practice talk: 45 minutes."}
---
...
**Prerequisites (read first if unfamiliar):** @sec-writing-manuscripts.

**See also:** @sec-writing-thesis, @sec-latex, @sec-jupyter, @sec-scripts-vs-notebooks.
:::
...
## Running theme: tell them what they'll know at the end that they don't know now, then deliver it {.unnumbered}
...
## Why a talk isn't a paper read aloud
...
## Know which kind of talk you're giving
...
## Slides that help instead of compete
...
Two books go much deeper: Garr Reynolds, *Presentation Zen*,^[Reynolds's book grew out of his Presentation Zen blog, which he started in 2005.] and Nancy Duarte, *slide:ology*.^[Duarte's first book, published in 2008. She worked with Al Gore on the slide show behind the documentary *An Inconvenient Truth*.] Both argue for high-image, low-text slides, with many examples (see Further reading).

## Structuring a fifteen-minute talk
...
## Presenting over Zoom and in hybrid rooms
...
## Choosing a slide tool

Any [presentation program](https://en.wikipedia.org/wiki/Presentation_program) can make a good talk or a bad one, and knowing yours well matters more than which one it is. **PowerPoint, Keynote, and Google Slides** are what most people use, and they're fine; for group work, pick one everyone can edit. **Beamer**, the LaTeX presentation class, is strong for math-heavy talks, and @sec-latex has a starter talk using the clean [metropolis theme](https://ctan.org/pkg/beamertheme-metropolis). **Quarto slides** (reveal.js)^[<https://quarto.org/docs/presentations/revealjs/>] suit anyone who already writes in Quarto or Jupyter (@sec-jupyter, @sec-scripts-vs-notebooks): slides are plain [Markdown](appendix-glossary.qmd#term-markdown) that can include code and its output. Each `##` heading starts a slide, and a `.notes` block holds speaker notes:
...
See @sec-artifacts-politics for the broader framework. The concrete prompt to carry forward: when you present, describe your figures out loud and use the microphone, and when you ask a question or chair a session, notice who hasn't spoken yet and make room for them.
...
## Quick reference: timing across formats
