...
meme:
  ...
  lines:
    - ""
    - "WHAT'S IN THE FILE?"
  alt: "What's in the box meme: What's in the file?"
# margin-header: generated from meme: by tools/chapter-meme/sync_margin_header.py; do not edit
margin-header: |
  ![](/graphics/memes/tabular-data.png){.chapter-meme fig-alt="What's in the box meme: What's in the file?"}
---
...
**Prerequisites (read first if unfamiliar):** @sec-data-file-formats.

**See also:** @sec-pandas-basics, @sec-sql-basics, @sec-tracebacks.
:::
...
This chapter is a field guide to the practices that catch those bugs early. It covers what shape your data should be in (wide vs. tidy), how to clean it without corrupting it, how to validate it with lightweight checks, and the specific friction points that burn the most student hours. It deliberately does not teach pandas syntax — that is @sec-pandas-basics — or SQL — that is @sec-sql-basics. Instead it teaches the *principles* that apply regardless of tool, so that when you pick up a new library you already know what good looks like.
...
## Running theme: data you have not validated is data you cannot trust {.unnumbered}
...
1.  **Load the raw file without modifying it.** Never overwrite the source file. The raw data is the ground truth you reproduce from. See @sec-data-file-formats.
...
9.  **Save the cleaned output to a separate file.** Use Parquet (see @sec-data-file-formats). Never save the cleaned data on top of the raw file.
...
This separation does three things: it makes cleaning reproducible, it makes the lineage of any number in your report traceable, and it prevents the dreaded "I don't remember which version of this file is the good one" situation. See @sec-project-management for how this fits into overall project structure.

## Validation: lightweight checks that catch the most bugs
...
## The seven most common friction points
...
Then fix the source: use `na_values=` at read time (see @sec-data-file-formats) or strip the junk explicitly.
...
See @sec-artifacts-politics for the broader framework. The concrete prompt to carry forward: every cleaning step is a claim that the rows you removed do not matter. Write down which rows you removed and *why* — that note is part of your analysis, not separate from it.
