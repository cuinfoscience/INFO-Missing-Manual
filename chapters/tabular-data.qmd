...
This chapter is about those habits: what shape a table should be in, how to clean one without damaging it, how to check it before you trust it, and the specific snags that eat the most student hours. It doesn't teach pandas syntax from scratch (that's @sec-pandas-basics) or SQL (@sec-sql-basics). The ideas here hold whichever tool you use, so they'll still be useful the next time you pick up a new one.

## Learning objectives {.unnumbered}

By the end of this chapter, you should be able to:

1.  Tell wide data from tidy (long) data, and explain when you want each.
2.  Explain the three rules of tidy data in your own words, with an example of each.
3.  Keep raw data untouched, and turn it into clean data with a script you can rerun.
4.  Walk through a cleaning pipeline in a sensible order, and say why the order matters.
5.  Write a short block of checks (shape, columns, types, keys, values) that stops an analysis when the data isn't what you expected.
6.  Recognize the seven snags that most often trip people up with a new dataset, and fix each one.

## Running theme: data you haven't checked is data you can't trust {.unnumbered}
...
## Wide and tidy: the shape of a table
...
**Finally, save the result to a new file,** ideally Parquet, which keeps your carefully fixed types (see @sec-data-file-formats). Never save it on top of the raw file.
...
Look at what it prints before deciding what to do. Often the fix belongs at read time, with `na_values=` (see @sec-data-file-formats), or in one line that strips the `$`.
