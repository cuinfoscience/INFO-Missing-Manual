...
meme:
  ...
  lines:
    ...
    - "ADVISOR LIKES IT"
    - "ADVISOR WANTS IT RESTRUCTURED"
  alt: "Panik-Kalm-Panik Meme: Drafted the introduction, Advisor likes it, Advisor wants it restructured."
# margin-header: generated from meme: by tools/chapter-meme/sync_margin_header.py; do not edit
margin-header: |
  ![](/graphics/memes/writing-manuscripts.png){.chapter-meme fig-alt="Panik-Kalm-Panik Meme: Drafted the introduction, Advisor likes it, Advisor wants it restructured."}
---
...
**Prerequisites (read first if unfamiliar):** @sec-reading-scholarship, @sec-asking-questions.

**See also:** @sec-writing-thesis, @sec-latex, @sec-git-github, @sec-project-management, @sec-ai-llm.
:::
...
## Running theme: write for a tired, smart, skeptical reader who has thirty other papers in their stack {.unnumbered}
...
Two companions are worth finding early. Wendy Belcher's *Writing Your Journal Article in Twelve Weeks*^[The second edition (University of Chicago Press, 2019) is the one to look for; it's listed in Further reading at the end of this chapter.] is a workbook that takes you from rough draft to submission a week at a time. Pat Thomson's blog *patter*^[Thomson is Professor Emerita of Education at the University of Nottingham and has supervised more than sixty PhDs, so her advice comes from a lot of drafts. The blog is linked in Further reading.] is a long-running, warm voice on what academic writing actually feels like. Read both while you draft, not after.

## Style: the conventions of academic prose
...
## Conferences vs. journals (in information-science-adjacent fields)
...
**Show your drafts early.** Belcher's "writing partners" idea is sound: a half-hour swap of pages with one peer each week does more for your writing than ten hours alone. @sec-collaboration has the etiquette.

**Keep the manuscript under version control.** A `manuscript/` folder with `main.tex`, `references.bib`, and the figures, tracked in Git, lets you get back yesterday's version and see what a co-author changed. See @sec-git-github for the workflow and @sec-latex for what belongs in `.gitignore`.
...
## Reading reviews and responding
...
## R&R, accept, reject: what to do next
...
**Accept.** Celebrate. Then read the [camera-ready](https://en.wikipedia.org/wiki/Camera-ready) instructions twice. For ACM venues they include the final template and ACM's TAPS production system, a rights form, an ORCID iD for every author, and deadlines for supplementary materials. @sec-latex covers the template mechanics.
...
- Does the bibliography compile cleanly with stable citation keys (see @sec-reading-scholarship)?
- Is the manuscript under version control with the `.bib` file (see @sec-git-github)?
- Did you check the AI-disclosure policy in the current call for papers?

## Quick reference: conference vs. journal in HCI/IS
...
- **ACM**, [Master Article Template](https://www.acm.org/publications/proceedings-template) — the official LaTeX and Word templates for ACM venues; pairs with @sec-latex.
