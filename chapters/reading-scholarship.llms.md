...
**A [Zettelkasten](https://en.wikipedia.org/wiki/Zettelkasten)** (“slip box”) is built from atomic notes: one idea per note, in your own words, linked to related notes. The sociologist [Niklas Luhmann](https://en.wikipedia.org/wiki/Niklas_Luhmann) kept one of some 90,000 index cards and credited it with his prolific writing. Today people build them in apps like Obsidian[^2] and Logseq.[^3] Its strength is synthesis: when you sit down to write a literature review, the connections are already there. Its risk is overhead: if you build the system and never write from it, you’ve built a very pretty filing cabinet.

**Annotation-first** means highlighting and commenting on the PDF itself, on the web with Hypothes.is[^4] or inside your library with Zotero’s [PDF reader](https://www.zotero.org/support/pdf_reader), which can gather a paper’s annotations into a note in one step. It’s low overhead and good for fast triage. The catch is that highlights aren’t knowledge: unless you go back and turn them into ideas in your own words, they rot.
...
Install Zotero[^5] and the Zotero Connector for your browser; the [Quick Start Guide](https://www.zotero.org/support/quick_start_guide) covers both. The Connector is the part that feels like magic: on a Google Scholar result, a publisher’s page, or an arXiv abstract, one click [saves the item](https://www.zotero.org/support/adding_items_to_zotero) with its authors, title, and journal, attaches the PDF when it can, and files it in a collection, with a popup where you can change the collection or add tags. Make a [collection](https://www.zotero.org/support/collections_and_tags) for each course or project, and tag freely by method, population, theory, or just `to-read`.

If you write in LaTeX or Markdown, add the **Better BibTeX** plugin.[^6] It gives every item a stable [citation key](https://retorque.re/zotero-better-bibtex/citing/), the short label like `keshav2007` that you type in `\cite{}` or `[@...]`, instead of recomputing keys at every export, and it can [keep a `.bib` file up to date](https://retorque.re/zotero-better-bibtex/exporting/auto/) automatically, so your bibliography is always current. The worked example below sets both up, and [sec-latex](#sec-latex) covers the writing side.
...
Large language models change the economics of reading, for better and worse. [sec-ai-llm](#sec-ai-llm) covers these tools in general; here’s what matters for scholarly reading.
...
A protocol that works: get a summary, do your own first pass, and compare. Where they disagree, the paper wins, and anything the model mentions that you didn’t see in the paper is suspect until you check it. [sec-evaluating-ai](#sec-evaluating-ai) has more on checking what these tools tell you.
