...
meme:
  ...
  lines:
    ...
    - "MY JUPYTER NOTEBOOK"
    - "WHY WOULD PYTHON DO THIS?"
  alt: "Who Killed Hannibal? Meme: Out-of-Order Cell Execution, My Jupyter Notebook, Why Would Python Do This?"
# margin-header: generated from meme: by tools/chapter-meme/sync_margin_header.py; do not edit
margin-header: |
  ![](/graphics/memes/jupyter.png){.chapter-meme fig-alt="Who Killed Hannibal? Meme: Out-of-Order Cell Execution, My Jupyter Notebook, Why Would Python Do This?"}
---
...
- **Reading more than you need, or the same file over and over.** Read only the columns you use, and save an expensive intermediate result to Parquet so the next run starts from it (see "Data bigger than memory" in @sec-data-file-formats).
...
## Notebooks on someone else's computer: Colab, Kaggle, and Codespaces
...
: Three hosted notebook platforms, and what changes on each. {#tbl-hosted-notebooks}
...
The code differs, but the idea is the one in @sec-secrets: the notebook names the secret and never contains it.
