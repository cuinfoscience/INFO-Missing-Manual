...
This chapter is a field guide to the five file formats you will meet most often as a novice data scientist — [CSV](appendix-glossary.qmd#term-csv), TSV, [JSON](appendix-glossary.qmd#term-json), Excel, and [Parquet](appendix-glossary.qmd#term-parquet) — with the specific quirks of each, the `pandas` reader invocation that will actually work, and the debugging steps for when it does not. The goal is not to make you a file format expert but to give you enough mental model to recognize what is going wrong when a simple read fails.
...
A [CSV](appendix-glossary.qmd#term-csv) file is a plain-text file where each line is a row and columns are separated by a delimiter — traditionally a comma. CSV is the most common data format you will encounter because it is human-readable, supported everywhere, and easy to generate from any tool.
...
[JSON](appendix-glossary.qmd#term-json) is the dominant format for data returned from web APIs. Unlike CSV, JSON supports nested data — a record can have fields whose values are themselves objects or arrays.
...
[Parquet](appendix-glossary.qmd#term-parquet) is a binary, columnar format designed for analytical data. It is the right answer for datasets bigger than a few hundred MB or for data you load repeatedly. You will need `pyarrow` installed:
