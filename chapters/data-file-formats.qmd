...
None of that means you're bad at this. pandas has to guess at the habits of whoever made the file (which character separates columns, which values mean "nothing here"), and when it guesses wrong, the fix is almost always one argument, once you know which one to reach for. This chapter is a field guide to the formats you'll meet most: [CSV](appendix-glossary.qmd#term-csv) (and its cousin TSV), [JSON](appendix-glossary.qmd#term-json), Excel, and [Parquet](appendix-glossary.qmd#term-parquet), plus the text encodings underneath them and what to do when a file is too big to load. It doesn't cover cleaning a table once it's loaded (that's @sec-tabular-data), pandas itself (@sec-pandas-basics), fetching JSON from an API (@sec-http-apis), or JSON as a configuration format (@sec-common-formats).
