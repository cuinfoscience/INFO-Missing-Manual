...
One `SyntaxError` catches nearly everyone once. You type `python`, and then, at Python’s `>>>` prompt, the install command from your instructions:
...
The command is fine; it’s in the wrong place. `pip install` is for your shell, the prompt that ends in `$`, `%`, or `>`, and `>>>` means you’re talking to Python, which tries to read the line as Python code. Leave Python with `exit()` (or `Ctrl + D`; on Windows, `Ctrl + Z` and then Enter) and run the command at the shell prompt. In a Jupyter notebook, where every cell is Python, run `%pip install pandas` in a cell instead. [sec-terminal](#sec-terminal) shows how to tell the prompts apart. (Python 3.13 and later name the file `<python-input-0>` instead of `<stdin>`.)
