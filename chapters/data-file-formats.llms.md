...
`pd.read_csv` treats a small set (`NA`, `N/A`, `NULL`, empty) as missing by default, but not all. The rest will sneak in as strings and silently poison any numeric operation (see the `ValueError: could not convert string to float` example in [sec-tracebacks](#sec-tracebacks)).
...
[JSON](../chapters/appendix-glossary.llms.md#term-json) is the dominant format for data returned from web APIs. Unlike CSV, JSON supports nested data — a record can have fields whose values are themselves objects or arrays.
...
[Parquet](../chapters/appendix-glossary.llms.md#term-parquet) is a binary, columnar format designed for analytical data. It is the right answer for datasets bigger than a few hundred MB or for data you load repeatedly. You will need `pyarrow` installed:
