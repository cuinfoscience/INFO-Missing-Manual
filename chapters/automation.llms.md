...
Now `make` does exactly the work that is out of date:
...
The second `make` does nothing, because nothing changed. After `report.py` changes, only the report is rebuilt; the cleaning step is skipped because its inputs are the same. The forgotten step on the checklist can’t be forgotten any more: change `clean.py`, and `make` reruns the report too.
...
It moves to its own folder, calls the environment’s Python by its path instead of relying on `activate`, and appends everything, errors included, to a dated log. Before scheduling it, run it the way a scheduler will: from another folder, with an empty environment. `cd / && env -i /bin/bash /path/to/coffee-sales/run_daily.sh` runs it with no `PATH` and no variables at all, and here it exited 0 with a log running from `start` to `done`. Then check the failure: with the raw file moved away, the script exited 1, and the log ended in the traceback, with no `done` line:

``` text
FileNotFoundError: [Errno 2] No such file or directory: 'data/raw/sales-2026-04-10.csv'
```

Only then add the schedule: on macOS or Linux, `crontab -e` and a line such as `15 6 * * * /path/to/coffee-sales/run_daily.sh`, which runs it at 6:15 every morning; on Windows, a Task Scheduler task (see “Scheduling scripts” above). Check the log the next morning, and again after the first day something goes wrong.

### Add GitHub Actions CI

CI runs a list of commands on a fresh computer, so try those commands on a fresh copy of your project before you push the workflow. Clone the repository into a new folder, install from `requirements.txt`, and run the steps from “A minimal CI workflow” above. For this project, the tests failed:
...
The test checks the processed file, and a fresh clone has none; `data/processed/` is ignored by Git. On your own computer the file was always there, which is why the test always passed. The fix is for CI to build what the test needs, which the Makefile already knows how to do: change the workflow’s test step from `pytest tests/ -q` to `make test`. In the fresh clone, `make test` runs `clean.py` first and then passes.

Commit the workflow as `.github/workflows/ci.yml` and push. The pull request page shows a check beside each commit, and the Actions tab shows each step, as [Figure fig-github-actions-run](#fig-github-actions-run) does for this book. When a step fails, open it, read the log from the bottom up, and reproduce the failure locally before changing anything.

### Speed up CI with caching

The minimal workflow already has the one line that matters most: `cache: pip` in the `setup-python` step. It saves the downloaded packages after a run, keyed on `requirements.txt`, and restores them on the next run, until `requirements.txt` changes. To see whether it helps, compare runs in the Actions tab, which lists each run’s duration. The first run after adding the cache is no faster, because it fills the cache; compare the second with a run from before. If the install step is only a few seconds either way, the project doesn’t need more caching, and a cache you don’t need is one more thing that can go stale.

### Add pre-commit hooks, and CI to enforce them

Install pre-commit with the configuration from “Local quality gates” above, then run `pre-commit autoupdate` before the first commit, so the hooks start at their current versions:
...
Pinned versions go stale, so run it again every few months. When this example was first run, the project’s configuration pinned the older releases shown, and the old `pre-commit-hooks` printed a deprecation warning on every run; the update silenced it. In `ruff-pre-commit`’s current releases the lint hook is called `ruff-check`, as in the configuration above; the old name `ruff` still works but is marked a legacy alias. Now a commit with trailing spaces, an unused import, and unformatted code is stopped and fixed:
...
Every hook fixed what it found, so `git add src/by_store.py` and the same commit command again succeeds, with every hook passing. Hooks run only on computers where someone ran `pre-commit install`, and `--no-verify` skips them, so add the same checks to CI as one more workflow step: `pre-commit run --all-files`.
...
Check it with a linter before you run it anywhere. [actionlint](https://github.com/rhysd/actionlint) checks workflow files (it installs with `pip install actionlint-py`), and it found three problems:
...
Then read it for what no linter checks. `python-version: 3.10` without quotes is the number 3.1 to YAML, so the job would ask for Python 3.1; write `"3.10"`. `on: push` never runs on pull requests, where review happens. `permissions: write-all` gives the job far more power than running tests needs; `permissions: contents: read` is enough (see “CI security basics” above). And `pytest tests/` has the missing-file problem from “Add GitHub Actions CI.” An AI draft saves typing; it doesn’t save you from reading every line, and a draft that looks finished is the one most worth checking.
