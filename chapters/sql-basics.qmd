...
## Running theme: SQL is a declarative language, so describe what you want, not how {.unnumbered}
...
Look at the first row: SQL gathered the NULL keys into a group of their own. pandas' `groupby` leaves them out unless you pass `dropna=False`, one of the silent row losses @sec-tabular-data warns about.
...
A join pairs each left row with *every* matching right row, so a key that appears twice on the right doubles its rows, and every sum over them. It's the pandas merge trap from @sec-tabular-data, with the same defense: count rows before and after, and check that the key is unique where you assume it is.
...
From here it's ordinary pandas (see @sec-pandas-basics). pandas accepts a `sqlite3` connection directly; other databases, such as PostgreSQL, usually go through SQLAlchemy. One surprise: an integer column that contains NULLs, like `orders.customer_id`, arrives as floats (`1.0`, `NaN`), because a NumPy integer column can't hold a missing value.
...
[DuckDB](https://duckdb.org/docs/stable/clients/python/overview) also runs inside Python with no server (`python -m pip install duckdb`), but it's built for analysis, and it runs SQL directly on CSV and Parquet files and pandas DataFrames, just by naming them (@sec-data-file-formats uses it on a file too big to load). Export the orders with `sqlite3 -header -csv shop.db "SELECT * FROM orders" > orders.csv`, and join that file to a DataFrame:
...
-   For anything beyond this chapter, your database's own documentation is the authority (see @sec-reading-docs).
