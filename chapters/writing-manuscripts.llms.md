...
That third contribution is a promise, so plan to keep it. A replication-ready release is the code, and the data or instructions for getting it, that produced every number in the paper, frozen at the version you submitted and archived somewhere it won’t disappear. [sec-git-github](#sec-git-github) shows how to tag a release on GitHub and have Zenodo archive it with its own DOI, which you can cite in the paper like any other source. A DOI and a GitHub link both name you, though, so during anonymous review the anonymized copy in the supplementary materials stands in for them (the next example shows one), and the DOI goes into the camera-ready version. Many journals also ask for a **data-availability statement**, a sentence or two saying where the data and code are and how to get them, or why they can’t be shared. Springer Nature, for one, [requires one](https://www.springernature.com/gp/authors/research-data-policy/data-availability-statements) in original research articles across its journals. For this paper it might read: “Code and the annotated comment sample are archived at Zenodo (DOI: 10.5281/zenodo.0000000). The full comment histories can’t be redistributed under the platform’s terms; the collection script and the list of comment IDs are included instead.”
...
Data-availability statement: where the data and code are and how to get them, or why they can’t be shared (DOI added at camera-ready if review is anonymous).
