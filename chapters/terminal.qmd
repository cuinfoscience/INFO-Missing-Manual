...
- **Git Bash** — if you have installed [Git for Windows](https://gitforwindows.org/) (see @sec-git-github), you already have it. Press the Windows key and type `git bash`. It needs no administrator rights, installs in minutes, and runs most of this chapter as written. This is the path of least resistance on a managed or locked-down laptop.
...
- **Anaconda Prompt** — if you installed Anaconda or Miniconda (see @sec-pkg-mgmt), this is the shell where `conda activate` works without extra configuration. Its file commands are Windows-native rather than Unix, so use it for environment work and one of the two above for everything else.
...
Every prompt you meet is built from those same pieces, even when the details differ between machines (@fig-macos-terminal).
...
This is the entire premise of shell scripting in one paragraph. The rest — control flow, error handling, exit-code conventions, and patterns for building real automation pipelines — lives in @sec-automation, which treats scripts as the bottom rung of a longer ladder. Read that chapter when you are ready to write scripts that other people will run.
...
The practical reason this matters is that many tools — databases, APIs, cloud SDKs — are configured through environment variables rather than through command-line flags. You do not type your database password on every command; you set `DATABASE_URL` once, and every tool that looks for it finds it. The discipline around secrets in environment variables (never commit them, never echo them into a shared log) lives in @sec-secrets.
...
See @sec-artifacts-politics for the broader framework. The concrete prompt to carry forward: the terminal is a powerful tool *and* a status marker; you can learn it well without pretending the gatekeeping around it is not real.
