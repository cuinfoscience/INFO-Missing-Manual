...
1.  Open a terminal on macOS, Windows, or Linux, and identify which shell you are running.

2.  Explain what a **terminal** and a **shell** are, and how commands run.

3.  Navigate the file system reliably (absolute/relative paths, current directory).

4.  Create, view, copy, move, and delete files and directories safely.

5.  Use help tools (`–help`, `man`) and interpret usage/syntax.

6.  Combine commands with pipes and redirection to build small workflows.

7.  Understand exit codes, errors, and common failure messages.

8.  Use permissions and `sudo` responsibly (least privilege, when *not* to use it).

9.  Apply basic security hygiene: protect secrets, avoid unsafe pastes, and reduce accidental damage.

## Scope note

This chapter focuses on Unix-like shells (macOS Terminal, Linux, and Windows via WSL or Git Bash). The section below on opening a terminal tells Windows readers how to get one of those; a section near the end maps concepts to Windows PowerShell where relevant.
...
None of the vocabulary in this section will stick until you have a terminal window open in front of you, so start there and read the rest with a prompt blinking on your screen.

### Opening a terminal

**macOS.** The terminal is already installed; you never need to download anything. The fastest way in is Spotlight: press `⌘ + Space`, type `terminal`, and press Enter. If you prefer to click, the application lives at **Applications → Utilities → Terminal**, or you can open Launchpad and look in the “Other” folder. Since macOS Catalina (10.15), the shell you land in is `zsh`; on older machines it is `bash`. Everything in this chapter works as written either way.

**Windows.** Windows has several terminals and several shells, and the combination you pick decides whether this chapter’s commands work verbatim. Windows 11 ships with **Windows Terminal**: press the Windows key, type `terminal`, and press Enter. On Windows 10, install Windows Terminal free from the Microsoft Store, or press the Windows key and type `powershell` to open PowerShell directly.

Windows Terminal is a window that hosts *other* shells in tabs — the `˅` next to the new-tab button lists every shell it found on your machine. What opens by default is **PowerShell**, which is a capable shell with its own command names (`Get-ChildItem` where this chapter says `ls`). Some Unix names work in PowerShell as aliases, but many of this chapter’s examples will not run there. To follow along, open one of these instead:

- **Git Bash** — if you have installed [Git for Windows](https://gitforwindows.org/) (see [sec-git-github](#sec-git-github)), you already have it. Press the Windows key and type `git bash`. It needs no administrator rights, installs in minutes, and runs most of this chapter as written. This is the path of least resistance on a managed or locked-down laptop.
...
|----|----|----|----|
| macOS | `⌘ + Space`, type `terminal` | `zsh` (or `bash`) | Work as written |
| Windows | Start → `terminal` | PowerShell | Many need translating |
| Windows | Start → `git bash` | `bash` | Work as written |
...
> **TIP:**
>
> You will open a terminal hundreds of times this semester. Pin it now: right-click its icon in the macOS Dock and choose **Options → Keep in Dock**, or right-click it on the Windows taskbar and choose **Pin to taskbar**.
...
1.  Open a terminal on your own machine. Run `echo $SHELL` and `pwd`, and write down which shell you are in and where it started you. If you are on Windows and `echo $SHELL` prints nothing, identify which shell you opened and get a `bash` prompt using one of the options above.

2.  Navigate from your home directory to a course folder using only `cd`, `pwd`, `ls`.

3.  Create a project directory structure with `mkdir -p`.

4.  Create a text file, preview it with `head`, then open it in `less`.

5.  Copy a directory; rename the copy; explain the difference between copy and move.

6.  Use a wildcard to list only `.csv` files; preview the expansion with `echo`.

7.  Build a pipeline that searches within a file and writes matching lines to an output file.

8.  Trigger a permission error intentionally (in a safe location) and explain why `sudo` is not the first fix.
