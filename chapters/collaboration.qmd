...
@fig-github-pr-review-comment shows a whole loop on one small thread: a line comment with a suggested change, a reply that says where the fix went, and the thread resolved. Notice one thing it doesn't do: the comment carries no label from the taxonomy above. The *Suggested change* box hints at what kind of comment this is, but a "**Suggestion**:" prefix would have said so outright.
