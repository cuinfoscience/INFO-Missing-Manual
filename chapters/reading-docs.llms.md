...
This chapter teaches that skill: finding the right kind of page, reading a function’s signature and docstring, pulling a working example out of the docs, and using release notes to explain code that “worked last year.” The examples come from [pandas](https://pandas.pydata.org/docs/) and Python itself, but the habits carry over to any library. It isn’t about *writing* documentation; that’s [sec-documentation](#sec-documentation).
...
The error in step 2 is the useful kind: it names exactly what’s wrong, and you already know it’s the column names because that’s the only thing that changed. This is the same move as building a [minimal reproducible example](https://en.wikipedia.org/wiki/Minimal_reproducible_example) when you ask for help (see [sec-asking-questions](#sec-asking-questions)), just run in the other direction. It’s slower than pasting a guess, for about five minutes. After that, you also understand *why* your call works, which is the difference between knowing a function and [cargo-culting](https://en.wikipedia.org/wiki/Cargo_cult_programming) it.
...
See [sec-artifacts-politics](#sec-artifacts-politics) for the broader framework. The concrete prompt to carry forward: when the docs you need are bad, ask why they’re bad before you blame yourself for not understanding them.
...
Thirty seconds, and a definitive answer: `"outer"` keeps every key from both tables, like a full outer [join](https://en.wikipedia.org/wiki/Join_(SQL)) in SQL (see [sec-sql-basics](#sec-sql-basics)), and sorts the keys. You also learned that the default is `"inner"`, which is why rows without a match have been disappearing from your merges. (And if you spotted the missing quote after `right_anti`, yes, that’s a typo in `pd.merge`’s real docstring; the `DataFrame.merge` page has it right. Official docs are written by people too.)
...
If the answer to all six is yes and you’re still stuck, it’s time to ask a question (see [sec-asking-questions](#sec-asking-questions)), and you’ll be able to say exactly what you’ve already tried.
