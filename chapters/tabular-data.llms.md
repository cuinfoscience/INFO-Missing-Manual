...
**Fix:** one bad value somewhere (a `"N/A"` or a `"$"` or a stray footnote) is making pandas treat the whole column as text (`str` in pandas 3, `object` in pandas 2). Use `pd.to_numeric(..., errors="coerce")` to convert and flag the culprits:
...
**Symptom:** a column has integers, floats, and a few strings (“N/A”, “n/a”, “unknown”). Reads as text (`str`, or `object` in pandas 2). Operations are slow and error-prone.
