...
If something like that has happened to you, you’re in good company, and there’s even a name for it: [dependency hell](https://en.wikipedia.org/wiki/Dependency_hell). The new library needed a newer NumPy, [pip](../chapters/appendix-glossary.llms.md#term-pip) upgraded NumPy to get it, and the older pandas your notebook relied on can’t work with the new NumPy. Nothing was wrong with your code. The trouble was in the pile of other people’s software your code stands on, and in the habit almost everyone starts with: installing everything into one shared Python and hoping for the best.

This chapter is about the habits that end that: a separate environment for every project, a small text file that records what the project needs, and a calm, step-by-step way to untangle a conflict when one happens. It covers both of the tools you’ll meet in data science, conda and pip, and when to use each. It doesn’t go deep on how a virtual environment works under the hood (that’s [sec-virtual-environments](#sec-virtual-environments)) or on connecting environments to Jupyter ([sec-jupyter](#sec-jupyter)), though it points to both where they matter.
...
Environments become [reproducible](https://en.wikipedia.org/wiki/Reproducibility) when you write down what’s in them, in a small text file you commit alongside your code. Conda’s version of that file is `environment.yml`; pip’s is `requirements.txt`. Both list package names and, optionally, version constraints. A stricter kind, a **lockfile**, records the *exact* [version](../chapters/appendix-glossary.llms.md#term-version) of every package the resolver chose.
...
`venv` is Python’s built-in environment tool, so any Python installation can make one without conda. The Python Packaging User Guide’s tutorial on [installing packages](https://packaging.python.org/en/latest/tutorials/installing-packages/) walks through the same steps as this section, and [sec-virtual-environments](#sec-virtual-environments) explains what a venv actually is on disk.
...
>
> If pip reports `ResolutionImpossible` above a list of conflicting constraints, you’ve hit a dependency conflict. Don’t start trying versions one at a time: “Dependency conflicts” and “A playbook for conflicts” below walk through what to do. See [sec-asking-questions](#sec-asking-questions) if you need to ask for help.

When pip installs something, it installs that package’s dependencies too, and the output shows all of them ([Figure fig-pip-install-success](#fig-pip-install-success)).

![Illustration of a terminal window running 'python -m pip install pandas'. The output collects pandas, shows a download progress bar, then collects numpy, python-dateutil, pytz, tzdata, and six, and ends with 'Successfully installed numpy-2.1.2 pandas-2.2.3 python-dateutil-2.9.0.post0 pytz-2024.2 six-1.17.0 tzdata-2024.2'. Numbered callouts mark the single package requested and the full list of packages actually installed.](../graphics/pip-install-success.png)

Figure 14.2: Illustration: A successful `python -m pip install pandas`. You asked for one package; the `Successfully installed` line reports six, because pip resolved and installed pandas’s dependencies as well.
...
A **lockfile** records the exact version of every package the resolver chose, transitive dependencies included, and sometimes a [checksum](../chapters/appendix-glossary.llms.md#term-checksum) of each file. A tool generates it after resolving, so installing from it is fast and predictable, and two collaborators installing the same lockfile get the same versions months apart. That ends “it resolved differently on my machine.”
...
When you do update, do it somewhere you can back out of: **a fresh environment, on a [branch](https://git-scm.com/docs/git-switch)** (see [sec-git-github](#sec-git-github)). Install the upgrade there and run your tests before you touch the environment you rely on. If something breaks, delete the test environment and carry on with the old one while you investigate. Upgrading in place and then finding a break can leave you with no working environment at all.
