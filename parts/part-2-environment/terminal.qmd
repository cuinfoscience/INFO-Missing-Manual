...
::: {.column-margin}
![ALT: macOS Terminal window at a fresh prompt, annotated to label the username, hostname, current directory, and prompt character. The window shows an `ls -l data/` command at the top and its output underneath.](graphics/PLACEHOLDER-macos-terminal-annotated.png){#fig-macos-terminal}
:::

::: {.column-margin}
![ALT: Windows Terminal with a PowerShell tab open, annotated to label the equivalent parts: current directory, prompt symbol, and command-line input area.](graphics/PLACEHOLDER-windows-terminal-annotated.png){#fig-windows-terminal}
:::
