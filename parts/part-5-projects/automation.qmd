...
meme:
  ...
  lines:
    ...
    - "ARE COMING"
  alt: "Your workflow file has been red for six commits. Each commit is one tab/space change. You have not yet run the actual job; the YAML parser keeps refusing."
  rationale: "frustration -- 'winter is coming' as the only honest YAML status report"
---
...
::: {.column-margin}
![ALT: GitHub Actions tab on a repository, showing a workflow run with a green checkmark beside each step of the job (checkout, setup-python, install dependencies, run tests).](graphics/PLACEHOLDER-github-actions-run.png){#fig-github-actions-run}
:::
