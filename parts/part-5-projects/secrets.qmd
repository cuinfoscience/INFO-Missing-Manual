...
meme:
  template: "oprah"
  lines:
    - "YOU GET A LEAKED API KEY"
    - "AND YOU GET A LEAKED API KEY"
  alt: "You committed `.env` to a public repo. GitHub emails you in twelve seconds. AWS emails you in two hours. The bill is $4,000 and counting."
  rationale: "humor -- Oprah's universal-distribution gesture, repurposed for catastrophe"
---
...
See @sec-git-github for the full `.gitignore` story.
...
2.  **Clear outputs before committing.** `Cell → All Output → Clear` in Jupyter Lab, or set up a git pre-commit hook with `nbstripout` to strip outputs automatically (the pre-commit framework is covered briefly in @sec-automation).
