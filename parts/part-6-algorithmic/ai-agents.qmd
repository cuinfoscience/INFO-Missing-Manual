...
This chapter builds on the model internals from @sec-llm-internals and the responsible-use practices from @sec-ai-llm. Where those chapters focus on individual prompts and outputs, this one focuses on *systems*: sequences of model calls, tool invocations, and decision points that can touch real data, real services, and real consequences.
...
**Unintended side effects.** The model calls a tool that modifies state (writes a file, posts to an API, sends a message) when it should not have. Fix: separate read tools from write tools; require explicit confirmation before executing write actions (see @sec-ai-llm for the risk-based policy).
...
4.  Consider an agent that can read files, write files, and send emails. Classify each tool as low-risk, medium-risk, or high-risk according to the framework from @sec-ai-llm. For the high-risk tools, describe what a human-in-the-loop checkpoint would look like.
