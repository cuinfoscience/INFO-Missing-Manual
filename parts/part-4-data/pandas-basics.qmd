...
meme:
  ...
  lines:
    - "`DF.LOC[5]`"
    - "`DF.ILOC[5]`"
  alt: "There are at least nine ways to select rows in `pandas`. Six of them give a `SettingWithCopyWarning`. The other three give different answers. You will pick the wrong one and spend twenty minutes finding out."
  rationale: "humor -- Kombucha Girl's intrigued/disgusted oscillation IS the .loc/.iloc decision face"
---
...
::: {.column-margin}
![ALT: Jupyter cell output showing a rendered pandas DataFrame. The rendering includes the row index on the left, bold column headers along the top, and several rows of data, illustrating the tabular structure a DataFrame provides.](graphics/PLACEHOLDER-dataframe-render.png){#fig-dataframe-render}
:::
