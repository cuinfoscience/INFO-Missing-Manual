...
**See also:** @sec-testing, @sec-scripts-vs-notebooks, @sec-asking-questions.
:::
...
It is a short chapter on purpose: reading tracebacks is a focused skill you can learn in 30 minutes and then reuse for the rest of your career. It pairs with @sec-debugging (which teaches the investigative loop) and @sec-testing (which teaches how to prevent the same error from coming back).
...
## 3. The ten exceptions you will see most often
...
## 4. Chained tracebacks: "during handling of the above exception"
...
## 5. Tracebacks in Jupyter notebooks
...
-   **The `Cell In[7]` header names the cell by its execution number**, not by the cell's position in the notebook. If you re-run cells out of order, these numbers can get confusing fast — see @sec-jupyter for why you should re-run from the top periodically.
...
## 10. Quick reference: common exceptions → first suspect
