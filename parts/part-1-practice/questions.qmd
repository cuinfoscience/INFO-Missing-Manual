...
meme:
  template: "awkward"
  lines:
    - "POSTS A QUESTION ON STACK OVERFLOW"
    - "MARKED DUPLICATE OF A 14-YEAR-OLD UNANSWERED THREAD"
  alt: "You finally worked up the nerve to ask for help -- on Slack, on Stack Overflow, in office hours. The reply is a request for a minimal reproducible example, which feels like another homework assignment on top of the homework."
  rationale: "anxiety -- the social cost of asking is the real barrier"
---
...
This field matters more than people expect, because sometimes the most useful answer is not "fix this error" but "there is an easier way to achieve what you actually want." If the helper only sees the symptom — say, a regex that does not match — they may help you debug the regex when the real answer is that you should not be using a regex at all (see @sec-regex). Stating the goal explicitly keeps that door open.
