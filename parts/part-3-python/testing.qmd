...
**Prerequisites (read first if unfamiliar):** @sec-scripts-vs-notebooks, @sec-debugging.

**See also:** @sec-tracebacks, @sec-git-github, @sec-automation.
:::
...
## Running theme: a test is just a question your code answers every time you run it {.unnumbered}
...
## 5. Fixtures: sharing setup between tests
...
## 6. Parametrization: one test, many inputs
...
## 7. Bug-reproducing tests: the regression lock
...
2.  You narrow it down to a minimal reproducible example (see @sec-debugging).
...
## 8. When to test (and when not to)
...
The fastest way to get value out of testing is to pick one or two key functions in a project and cover those well, not to chase 100% coverage on everything. See @sec-project-management for how this fits into a project's overall quality gates.
...
7.  Add a `pytest` invocation to a CI workflow (see @sec-automation). Intentionally break one test and confirm the CI build fails.
