...
A handbook that teaches students how to use AI tools responsibly would be hypocritical if it quietly pretended its own authors did not. This appendix is a transparent record of how large language models were used in drafting, editing, and maintaining the *Paratechnical Computing Handbook*, plus a discussion of the editorial choices we made about what the AI was and was not allowed to do.
...
## B.1 1. What AI tools were used

During the drafting and conversion of this handbook we used:

- **Claude (Anthropic)**, accessed through Claude Code, to draft prose, convert LaTeX source files into Quarto, and assist with editing passes. Claude wrote first drafts of several chapters in [sec-tracebacks](#sec-tracebacks), [sec-virtual-environments](#sec-virtual-environments), [sec-testing](#sec-testing), [sec-data-file-formats](#sec-data-file-formats), [sec-tabular-data](#sec-tabular-data), [sec-regex](#sec-regex), [sec-http-apis](#sec-http-apis), [sec-secrets](#sec-secrets), [sec-reading-docs](#sec-reading-docs), [sec-linting](#sec-linting), [sec-pandas-basics](#sec-pandas-basics), [sec-sql-basics](#sec-sql-basics), [sec-pre-commit](#sec-pre-commit), and this appendix.
...
## B.2 2. What the AI was asked to do
...
## B.3 3. What the AI was *not* asked to do
...
- **Claims about specific authors, papers, or attributions.** Bibliography entries were curated and verified by the human authors. When the models invented a plausible-sounding citation, we removed it. See section 5.
...
## B.5 5. What we got wrong, and what we watched for
...
## B.6 6. What this means for students using the book
...
## B.7 7. Discussion: why we disclose at all
...
**For students.** One of the skills this book is trying to build is a literate relationship with AI tools: when to use them, how to verify their output, how to read around their mistakes, and how to disclose them when they are part of your own work (see [sec-ai-llm](#sec-ai-llm)). If we failed to model that ourselves, the advice in those chapters would be hypocritical. Disclosure is the worked example for the standard we are asking students to adopt.
...
## B.8 8. A note on the code we ship
...
If this book grows to include executable code cells (see [sec-jupyter](#sec-jupyter)) or tests (see [sec-testing](#sec-testing)) that ship with the book itself, the same standard will apply to them: AI may draft, humans verify and take responsibility.

## B.9 9. How we will keep this appendix current
...
## B.11 11. Checklist for your own work
